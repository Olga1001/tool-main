(function ($) {
  ...
  function chatWidget({ widgetMetadata, myChatWidget, lang = "ua" } = {}) {
    ...
    let intervalId = null;
	let storangeIsMessageAssistent = localStorage.getItem('repliedMessageAssistent');
    let repliedMessageAssistent = storangeIsMessageAssistent || false ;
    ...
    function waitAssistentMessage() {
      let start = setInterval(() => {
        if ($('.user-message').length && $('.my_chat_bot').css('display') == 'none' && repliedMessageAssistent == false) {
    ...
    function sendMessage() {
	  ...
	  console.log('wp_context: ', wpСontext)
    ...
    function aiMessageHtml(message) {
 		...
 		// marked.parse(message.replace(/&shy;/g, ""));
