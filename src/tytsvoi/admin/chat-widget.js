(function ($) {
  ...
  function chatWidget({ widgetMetadata, myChatWidget, lang = "ua" } = {}) {
    ...
    var widgetBody = `
      ...
      ${createSocialLinks(socialLinks)}
            <div class="chat-messages-container-wrap">
                <!-- Chat Container -->
                <div id="chat-messages-container" class="pr-4 overflow-y-auto" style="min-width: 100%; max-height: inherit;">
    ...
    let intervalId = null;
    let intervalAssistentMessage = null;
    let repliedMessageAssistent = JSON.parse(localStorage.getItem('repliedMessageAssistent')) || false ;
    let waitSendMessage = false;
    let visibleMessangeAssistent = JSON.parse(localStorage.getItem('visibleMessangeAssistent')) || false;

    function addStorageRepliedAssistent(bool) {
      repliedMessageAssistent = bool;
      localStorage.setItem('repliedMessageAssistent', repliedMessageAssistent);
    }

    function addStorageVisibleAssistent(bool) {
      visibleMessangeAssistent = bool;
      localStorage.setItem('visibleMessangeAssistent', visibleMessangeAssistent);
    }

    function waitAssistentMessage() {
      let checkAssistentMessage = setInterval(() => {
        let selectorUser = $('.my_chat_bot .user-message');

        if (selectorUser.length &&
            $('.my_chat_bot').css('display') == 'none'
        ) {

          let selectorUserLast = selectorUser.last();
          let selectorAiNext = selectorUserLast.next();
          
          if (repliedMessageAssistent == true) {
            clearInterval(checkAssistentMessage);
            console.log('repliedMessageAssistent == true')

            if (selectorAiNext.length === 0) {
              console.log(selectorAiNext.length)
              console.log('selectorAiNext NONE (1)')
              console.log(selectorUserLast.find('.user-text').text())

              waitSendMessage = false;

              sendMessage(selectorUserLast.find('.user-text').text());
              intervalAssistentMessage = setInterval(waitAssistentMessage, 2000);
            }
          } else {
            console.log('repliedMessageAssistent == false')
            if ($('.assistant_typing').length === 0 && 
                waitSendMessage == false &&
                selectorAiNext.length === 0
            ) {
              waitSendMessage = true;

              console.log('selectorAiNext NONE (2)')
              console.log(selectorUserLast.find('.user-text').text())
          ...
          if (visibleMessangeAssistent == false &&
            repliedMessageAssistent == true
          ) {
    ...
    function initChatWidgetUI() {
      ...
      var toggleChatBtn = $("<button>", {
        class: `fixed bottom-4 ${positionChatWidget}-4 inline-flex items-center justify-center text-sm font-medium disabled:pointer-events-none disabled:opacity-50 rounded-full w-13 h-13 hover:opacity-80 m-0 cursor-pointer bg-none p-0 normal-case leading-5 hover:text-white btn-widget`,
      ...
      var chatWidget = $("<div>", {
        class: `my_chat_bot fixed bottom-[calc(4rem+1.5rem)] ${positionChatWidget}-0 ml-4 mr-4 bg-white p-6 rounded-lg border border-[#e5e7eb] w-[440px] h-[634px]`,
        style:
      ...
      function toggleWidget() {
        ...
        addStorageVisibleAssistent(true);
      ...
      $.ajax({
        ...
        success: function ({ data }) {
          ...
          insertMessageToChat(aiMessageHtml(message));
		      insertImagesToChat();
          addStorageRepliedAssistent(true);
        
          addStorageVisibleAssistent($('.my_chat_bot').css('display') !== 'none');
    ...
    function sendMessage(mess = '') {
      ...
      let message = mess == '' ? messageInput.val().trim() : mess;
      let noticesContainer = $(".mcb-notices");
	    let wpСontext = $('h1').length ? ($('h1').text() + `(${location.href})`) : location.href;

      if (message.length == 0) {
        chatWidget.find('button').removeAttr('disabled');
        return;
      }
		
	    addStorageRepliedAssistent(false)
      ...
      formData.append("wp_context", wpСontext);
	...
	async function getImages(link) {
		...
		// URL cleaning and decoding
		let url = decodeURIComponent(match[2].replace(/\$shy;/g, ''));
		url = url.replace(/%C2%AD/g, '').replace(/\$shy;/g, '');
		console.log('Fetching URL:', url);

		try {
			// Check for a page in the HEAD-post help
			...
			// Load the content page if the page have
			const pageResponse = await fetch(url);
			const text = await pageResponse.text();

			// Create a temporary container for parse HTML
			const tempDiv = document.createElement('div');
			tempDiv.innerHTML = text;

			// Get image product
			const imageElement = tempDiv.querySelector('.woocommerce-product-gallery img');
			const imageUrl = imageElement ? imageElement.src : '';
			const imageAlt = imageElement ? imageElement.alt : '';

			// Add an image before the text link
			if (imageUrl) {
	...
	function parseAiMessage(message) {
		// Remove unnecessary symbols with HTML
		let text = message.replace(/\$shy;/g, '');
// 		text = decodeURIComponent(text);

		// Remove numbering before link
		text = text.replace(/^\d+\.\s*/gm, ''); // Remove numbering format "1. "
		...
		imagePromises = links.map(link => getImages(link));
		
		return text.replace(linkPattern, '<p class="item-product"><a href="$2"><b>$1</b></a></p>');
	}
		
    function aiMessageHtml(message) {
      	if (!message) return "";

 		console.log(marked.parse(message)); //.replace(/&shy;/g, "")
	...
	function insertImagesToChat() {
		Promise.all(imagePromises).then(images => {
			$('.ai-message').last().find('.item-product').each((index, item) => {
				const imageHtml = images[index] || '';
				$(item).prepend(imageHtml);
			});
